...
(function () {
  ...
  var start0 = null, GATHER_MS = 8000;                   // 集合にかける時間（さらにゆっくり）
  ...
  function makeFruitPoint() {
    if (Math.random() < 0.86) {                       // 球体ボディ（表面寄りで密に）
      var th = Math.random() * Math.PI * 2, ph = Math.acos(2 * Math.random() - 1);
      var rr = Math.pow(Math.random(), 0.4) * R, sp = Math.sin(ph);
      return [sp * Math.cos(th) * rr, Math.cos(ph) * rr, sp * Math.sin(th) * rr, Math.random() < 0.07];
    }
    // 葉（3枚：中央＝直立、左右＝外へ傾く）— 小さめ
    var li = (Math.random() * 3) | 0, gap = R * 0.16, anchorX, anchorY, ang;
    if (li === 0) { anchorX = 0; anchorY = -(R + gap); ang = 0; }
    else if (li === 1) { anchorX = -0.22 * R; anchorY = -(R + gap * 0.7); ang = -0.95; }
    else { anchorX = 0.22 * R; anchorY = -(R + gap * 0.7); ang = 0.95; }
    var Lf = 0.66 * R, hWmax = 0.13 * R, thick = 0.06 * R;
    var t = Math.random();
    var halfW = hWmax * Math.pow(Math.sin(Math.PI * t), 0.7);
    var across = (Math.random() * 2 - 1) * halfW, along = t * Lf;
    var dirx = Math.sin(ang), diry = -Math.cos(ang), perpx = Math.cos(ang), perpy = Math.sin(ang);
    var lx = anchorX + along * dirx + across * perpx;
    var ly = anchorY + along * diry + across * perpy;
    var edge = halfW > 0 ? across / halfW : 0;
    var lz = (Math.random() * 2 - 1) * thick * Math.sqrt(Math.max(0, 1 - edge * edge));
    return [lx, ly, lz, Math.random() < 0.05];
  }

  function build() {
    var mobile = W < 760;
    var N = mobile
      ? Math.max(2600, Math.min(5200, Math.floor((W * H) / 64)))   // モバイルは負荷を抑える
      : Math.max(7000, Math.min(16000, Math.floor((W * H) / 52)));
    parts = [];
    cx = W * 0.5; cy = H * 0.52; R = Math.max(42, Math.min(W, H) * 0.11);
    function add(px, py, pz, sz, seed) {
      parts.push({ x: px, y: py, z: pz, size: sz, seed: seed, delay: Math.random() * 0.45,
    ...
    maxParts = parts.length + (W < 760 ? 1800 : 4500);   // カーソルで増やせる上限
  ...
  function frame(now) {
    ...
    // 点が集まり切った瞬間に見出しを出す
    if (!gatheredFired && gT >= 0.9 && heroEl) { gatheredFired = true; heroEl.classList.add('gathered'); }
    ...
    var scaleF = 1 + beatAmp * pulse * beatAmt * (1 + 0.9 * exciteS);   // 近いほど大きく脈打つ

    var rotY = Math.sin(tnow * 0.00035) * 0.55;               // ゆるやかな3D回転（左右に振れる）
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      if (gT < 1) {
        var e = easeOutCubic(clamp01((gT - p.delay) / (1 - p.delay)));
  ...
  if (reduce) {
    ...
    if (FREEZE !== null) {
      ...
      if (heroEl) heroEl.classList.add('prehold');   // 集合完了まで見出しを伏せる
